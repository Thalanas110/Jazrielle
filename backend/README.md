...
The command endpoint supports the actions declared in `ai/system-prompt.md`: applications, system status and metrics, time/date/weather, media and volume, reminders, updates, project controls, Git status, validated web URLs, server-side TinyFish Search/Fetch lookups, and conversation. Web searches return fetched result text in the command response and do not open a browser. TinyFish Agent and Browser APIs are not used. The backend validates every model response before dispatching it to a registered handler.
