...
The command endpoint supports the actions declared in `ai/system-prompt.md`: applications, system status and metrics, time/date/weather, media and volume, reminders, updates, project controls, Git status, validated web URLs, server-side TinyFish Search/Fetch lookups, and conversation. Web searches pass fetched content as untrusted reference material to a second local Qwen generation, which returns the concise answer in the command response. TinyFish Agent and Browser APIs are not used, and no browser is opened. The backend validates the intent model response before dispatching it to a registered handler.
...
The API key is required for both `api.search.tinyfish.ai` and `api.fetch.tinyfish.ai`; never commit it.
